...
tr_ts_summary_path = "./results" + feature + "/train_test_summary_5folds_5seeds.txt"
##use GPU##
GPU = True
...
ls_mb_size = [{'mbS': mbS, 'mbT': mbT}]  # Set the batch size for the best model manually
...
class_sample_count = np.array(
    [Counter(YGDSC['response'])[0] / len(YGDSC['response']), Counter(YGDSC['response'])[1] / len(YGDSC['response'])])
...
for index, mbsize in enumerate(ls_mb_size):
    ...
    IG_source_val_results_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(
        epoch) + '_lambda1' + str(lam1) + \
                                 '_mbS' + str(mbS) + '_mbT' + str(mbT) + '_seed' + str(
        init_seed) + '_attribution_source_val.tsv'

    IG_target_test_results_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(
        epoch) + '_lambda1' + str(lam1) + \
                                  '_mbS' + str(mbS) + '_mbT' + str(mbT) + '_seed' + str(
        init_seed) + '_attribution_target_test.tsv'
    ...
    for split in ls_splits:  # for each split
        ...
        if adjust == "Y":
            ...
            intersect_gene.sort()
            # intersect_gene = [x for x in XValGDSC.columns if x in frozenset(X_5k_cells.columns)]
        ...
        Map = MTLP(dropout_mtl, h_dim, z_dim)  ## drug response predictor ##
        ...
        for it in range(epoch):
            ...
            for i, data in enumerate(zip(cycle(CLoader), PLoader)):
                ...
                Map.train()  ### Mapper (aka drug response predictor)
                ...
                ###T-SNE feature concate###
                F_xs_cpu = F_xs.cpu().detach().numpy()
                F_xt_cpu = F_xt.cpu().detach().numpy()
                ys_cpu = ys.cpu().detach().numpy()
                yt_cpu = yt.cpu().detach().numpy()

                feature_s = F_xs_cpu if i == 0 else np.concatenate((feature_s, F_xs_cpu), axis=0)
                feature_t = F_xt_cpu if i == 0 else np.concatenate((feature_t, F_xt_cpu), axis=0)

                label_s = ys_cpu if i == 0 else np.concatenate((label_s, ys_cpu), axis=0)
                label_t = yt_cpu if i == 0 else np.concatenate((label_t, yt_cpu), axis=0)
            ...
            feature_cb = np.concatenate((feature_s, feature_t), axis=0)
            label_cb = ls_s + ls_t

            save_tsne_dir = SAVE_TRACE_TO + batch_sizes + '/' + model_params + '/plots/'
            if not os.path.exists(save_tsne_dir):
                os.makedirs(save_tsne_dir)
                print("Directory ", save_tsne_dir, " Created ")
            save_tsne_plot_to = os.path.join(save_tsne_dir, split + '_tsne_epoch' + str(it) + '.jpeg')
            if it == 0 or it % 10 == 9 or it == epoch - 1:
                plot_tsne(feature_cb, label_cb, save_tsne_plot_to, it)
        ...
        with open(test_predicted_y_file, 'a') as p:
            ...
            for tp in range(TYTestCells.shape[0]):
                p.write("{}\t{}\t{}\t{}\n".format(split, YTestCells.index[tp], YTestCells['response'].values[tp],
                                                  test_predicted_y[tp][0]))
        ...
        XIG.requires_grad_()
        attr, delta = ig.attribute(XIG, target=0, return_convergence_delta=True, n_steps=100)  # TXTestCells_N
        ...
        if torch.equal(XIG, TXValGDSC_N):
            ## save IG attribution values for source validation set
            attr_pd.to_csv(IG_source_val_file, sep="\t", index=False)
        elif torch.equal(XIG, TXTestCells_N):
