...
parser.add_argument('--lam1','-la1',help='lam1',type=float, nargs='?', default=0)
parser.add_argument('--mbS','-mbS',help='mbS',type=int)
parser.add_argument('--mbT','-mbT',help='mbT',type=int)

parser.add_argument('--cell_lines','-l',help='use all cell lines or solid cell line only, should be either one of '
                                             '<all, solid>', nargs='?', default="all")
#
...
if cell_lines == "solid":
    tr_ts_summary_path = "./results_solid" + feature + "/train_test_summary_5folds_5seeds_solid.txt"
...
#####################################################
#        Baseline Model Training Starts Here        #
#####################################################
for index, mbsize in enumerate(ls_mb_size):
    ...
    test_results_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(epoch) + \
    ...
    test_predicted_y_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(
        epoch) + '_mbS' + str(mbS) + '_mbT' + str(mbT) + '_seed' + str(init_seed) + '_predicted_y.tsv'
    ...
    with open(test_results_file, 'a') as f:
        f.write("-- Parameters --\n\n")
        f.write("h_dim: {}\nz_dim: {}\nlr: {}\nepoch: {}\nmbS: {}\nmbT: {}\n".format(h_dim,z_dim,lr,epoch,mbS,mbT))
    ...
    ####
        model_params = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(epoch) \
                        + '_mbS' + str(mbS) + '_mbT' + str(mbT)

        ### train the feature extractor and drug predictor sorely on bulk RNA-seq data
        XTrainCombined = XTrainGDSC
        ...
        optimizer_2 = torch.optim.Adagrad(itertools.chain(Gen.parameters(), Map.parameters()), lr=lr)
