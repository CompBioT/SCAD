...
feature=geneset
if adjust=="Y":
    X_5k_cells_raw = pd.read_csv(WDR+"/ANN/BMDSTL/BM/data/original_norm/Pancan_CCLE_10X/scrna_ccle_CPM_random_5k_raw_x_scaled_ncbi.tsv", sep='\t', index_col=0, decimal='.')
...
for index, mbsize in enumerate(ls_mb_size):
    ...
    test_predicted_y_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(epoch) + '_lambda1' + str(lam1) + \
                        '_mbS' + str(mbS) + '_mbT' + str(mbT) + '_smote' + '_seed' + str(init_seed) + '_predicted_y.tsv'
    ...
    ###
        ...
        l2 = []
        classif = []
        #aucs_tr_target = [] ## AUC for target data, not used in training ##
        L = [] # total loss
        DG_losstr = []
        DG_auctr = []
        ...
        for it in range(epoch):
            ...
            for i, data in enumerate(zip(cycle(CLoader), PLoader)):
                ...
                Gen.train()          ### Gen = EN(dropout_gen, IE_dim, h_dim, z_dim)     #### varient autoencoder or other gene feature extractor
                Map.train()          ### mapper
