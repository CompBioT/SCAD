...
drugls = ['NVP-TAE684','Afatinib','Sorafenib', 'Cetuximab']
DRUG ='Cetuximab'
cellline = 'scc47'
Suffix = ['scrna_ccle']
...
#(hnscc_sc_meta['EpiSen_score'].quantile(0.4) <= hnscc_sc_meta['EpiSen_score']) & (hnscc_sc_meta['EpiSen_score'] <= hnscc_sc_meta['EpiSen_score'].quantile(0.5))
...
# cells_EpiSen_low = hnscc_sc_meta.index[(hnscc_sc_meta['EpiSen_score'].quantile(0.4) <= hnscc_sc_meta['EpiSen_score']) & (hnscc_sc_meta['EpiSen_score'] < hnscc_sc_meta['EpiSen_score'].quantile(0.5))]
# cells_EpiSen_high = hnscc_sc_meta.index[(hnscc_sc_meta['EpiSen_score'].quantile(0.5) < hnscc_sc_meta['EpiSen_score']) & (hnscc_sc_meta['EpiSen_score'] <= hnscc_sc_meta['EpiSen_score'].quantile(0.6))]
EpiSen_select = []
...
if DRUG == 'NVP-TAE684' or DRUG == 'Afatinib' or DRUG == 'Cetuximab':
...
print("SCC47: Num of resistant and Num of Sensitive = {}".format(Counter(cell_sensitivity)))
...
print("SCC47 sample size and number of genes = {}".format(hnscc_exprs_z.shape))

# check overlap genes
GDSC_exprs_drug_z_genes = list(GDSC_exprs_drug_z.columns.values)
hnscc_exprs_z_genes = list(hnscc_exprs_z.columns.values)

if not (GDSC_exprs_drug_z_genes == hnscc_exprs_z_genes):
...
hnscc_exprs_z_resp = shuffle(hnscc_exprs_z_resp, random_state=42)
