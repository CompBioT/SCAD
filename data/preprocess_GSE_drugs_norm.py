...
if DRUG == 'Docetaxel' and GSEid == 'GSE140440':
    GSE_samples = []
    #####
    dict_gse140440 = {}
    import csv
    tsv_file = open("D:/Project2/ANN/BMDSTL/BM/data/original/Docetaxel/exprs/GSE140440_sampleinfo.txt")
    read_tsv = csv.reader(tsv_file, delimiter="\t")
    for row in read_tsv:
        dict_gse140440[row[0]] = row[1]
    #####
    for samp in GSE_exprs_z.columns:  # row names
        sampleid = samp.split("_")[0]
        GSE_samples.append(sampleid)
        if dict_gse140440[samp].startswith('DU145_Sen'):
            response = 1
        elif dict_gse140440[samp].startswith('DU145_Res'):
            response = 0
        elif dict_gse140440[samp].startswith('PC3_Sen'):
            response = 1
        elif dict_gse140440[samp].startswith('PC3_Res'):
            response = 0
        else:
            raise NotImplementedError('DRUG {} and GSEid {} not match'.format(DRUG, GSEid))
        print("{} and {}".format(samp,response))
        GSE_response.append(response)
    GSE_exprs_z.columns = GSE_samples

#elif DRUG == 'PLX4720' and GSEid == 'GSE108383':
elif DRUG.startswith('PLX4720') and GSEid == 'GSE108383':
    ...
    for samp in GSE_exprs_z.columns:
        ...
        GSE_response.append(response)
elif DRUG == 'Erlotinib' and GSEid == 'GSE134839':
    for samp in GSE_exprs_z.columns:
        response = []
        if "PC9D0-" in samp:
            response = 1
        ...
        if "PC9D11-" in samp:
            response = 0
        #elif ("PC9D2" in samp) or ('PC9D4' in samp) or ('PC9D9' in samp) or ('PC9D11' in samp):
        #    response = 0 ## 0 means resistant
        #else:
        #    raise NotImplementedError('samp {} and response {} not match'.format(samp, response))
        GSE_response.append(response)
        print("id: {}, label: {}".format(samp,response))

elif DRUG == 'Erlotinib' and GSEid == 'GSE149214':
    for samp in GSE_exprs_z.columns:
        response = []
        if "PC9D0" in samp or "PC9D0C" in samp:
            response = 1 ## 1 means sensitive
        elif "PC9D11C" in samp or 'PC9D11ERL' in samp:
            response = 0 ## 0 means resistant
        else:
            raise NotImplementedError('samp {} and response {} not match'.format(samp, response))
        GSE_response.append(response)
elif DRUG == 'Erlotinib' and GSEid == 'GSE134836':
    for samp in GSE_exprs_z.columns:
        response = []
        if "PC9D0" in samp:
            response = 1 ## 1 means sensitive
