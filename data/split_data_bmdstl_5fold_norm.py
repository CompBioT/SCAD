...
#Afatinib AR-42 Cetuximab Etoposide Gefitinib NVP-TAE684 PLX4720 Sorafenib Vorinostat

DRUG = "PLX4720"  # Set to the name of the drug being split
...
#D:\Project2\ANN\BMDSTL\BM\data\split\Docetaxel\stratified\source_5_folds
...
###########################################################################################
#D:\Project2\ANN\BMDSTL\BM\data\split\Docetaxel
if Solid=="False":
    source_exprs_resp_z = pd.read_csv('./data/split_norm/'+ DRUG + '/Source_exprs_resp_z.'+DRUG+geneset+'.tsv',
                                sep='\t', index_col=0)
## if use solid tumor only
if Solid=="True":
    source_exprs_resp_z = pd.read_csv('./data/split_norm/' + DRUG + '/Source_solid_exprs_resp_z.' + DRUG + geneset + '.tsv',
                                      sep='\t', index_col=0)

# Target data #
target_combined_exprs_resp_z = pd.read_csv('./data/split_norm/'+ DRUG + '/Target_expr_resp_z.'+DRUG+geneset+'.tsv',
                                sep='\t', index_col=0)

###############
GSE108383_10X = False
if GSE108383_10X == True and DRUG == "PLX4720":
    source_exprs_resp_z = pd.read_csv('./data/split_norm/' + DRUG + '_10X' + '/Source_exprs_resp_z.' + DRUG + '.tsv',
                                      sep='\t', index_col=0)
    # Target data #
    target_combined_exprs_resp_z = pd.read_csv('./data/split_norm/' + DRUG + '_10X' + '/Target_expr_resp_z.' + DRUG + '_10X' + '.tsv',
                                               sep='\t', index_col=0)
    SAVE_RESULT_TO_STRAT = './data/split_norm/' + DRUG + '_10X' + '/stratified/'
