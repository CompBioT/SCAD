...
tr_eva_ts_summary_path = './results'+feature+'/test_on_target_summary_5fds_smote_fixlr.txt'
tr_eva_on_source_summary_path = './results'+feature+'/train_eval_on_source_hypers_summary_5fds_smote_fixlr.txt'

if cell_lines == "solid":
    tr_eva_ts_summary_path = './results_solid'+feature+'/test_on_target_summary_5fds_smote_fixlr.txt'
    tr_eva_on_source_summary_path = './results_solid'+feature+'/train_eval_on_source_hypers_summary_5fds_smote_fixlr.txt'
...
ls_extractors = ['FX','FX_SFMX','FX_MLP','FX_MCA','FX_MLP_MCA', 'FX_VAE']
...
smi_filepath = WDR+"/ANN/AILT/paccmann/data/smiles/gdsc_append.smi"
smiles_language_filepath = WDR+"/ANN/AILT/paccmann/data/smiles_language_chembl_gdsc_ccle.pkl"
smiles_language = SMILESLanguage.load(smiles_language_filepath)

# store smile info with drug name
dict_smi = {}
import csv

tsv_file = open(WDR+"/ANN/AILT/paccmann/data/smiles/gdsc_append.smi")
read_tsv = csv.reader(tsv_file, delimiter="\t")

index = 0
for row in read_tsv:
    dict_smi[row[1]] = index
    index += 1

# Process parameter file:
import json
params = {}
with open(WDR+"/ANN/AILT/paccmann/paccmann_predictor_master/examples/IC50/example_params_append.json") as fp:
...
def predict_label(XTestPatients, gen_model, map_model):
    ...
    if extractor == 'FX' or extractor == 'FX_SFMX' or extractor == 'FX_MLP':
        F_xt_test = gen_model(XTestPatients.to(device))

    elif extractor == 'FX_VAE':
        F_xt_test = gen_model(XTestPatients.to(device))[0]

    elif extractor == 'FX_MCA' or extractor == 'FX_MLP_MCA':
        F_xt_test = gen_model( (smiles_dataset[dict_smi[DRUG]].cpu()).tile(XTestPatients.shape[0], 1).to(device), XTestPatients.to(device) )
    else:
...
ls_vae_lam = [0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8]
...
for index, mbsize in enumerate(ls_mb_size):
    ...
    for iters in range(MAX_ITER):
        ...
        lambdvae = random.choice(ls_vae_lam)
        ...
        test_results_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(
            epoch) + '_lambda1' + str(lam1)  + '_dropouts' + str(dropout_gen) \
                            + '_' + str(dropout_mtl) + '_' + str(dropout_dg)  + '_mbS' + str(mbS) + '_mbT' + str(mbT) + '.tsv'
        if extractor == 'FX_VAE':
            test_results_name = 'hdim' + str(h_dim) + '_zdim' + str(z_dim) + '_lr' + str(lr) + '_epoch' + str(
            epoch) + '_lambda1' + str(lam1)  + '_lambdavae' + str(lambdvae) + '_dropouts' + str(dropout_gen) \
                            + '_' + str(dropout_mtl) + '_' + str(dropout_dg)  + '_mbS' + str(mbS) + '_mbT' + str(mbT) + '.tsv'
        ...
        hyper_para_record = test_results_name
        ...
        AUCtest_splits_total = []  ## on target set
        ...
        for split in ls_splits:  # for each split
            ...
            if adjust == "Y":
                #X_5k_cells = pd.read_csv("./data/Pancan_CCLE_10X/scrna_ccle_CPM_random_5k_raw_x_scaled_ncbi.tsv",sep='\t', index_col=0, decimal='.')
                ...
                XTrainPatients = XTrainPatients[intersect_gene]
                XTestPatients = XTestPatients[intersect_gene]
            ...
            CDataset = torch.utils.data.TensorDataset(torch.FloatTensor(XTrainGDSC_N),torch.FloatTensor(YTrainGDSC.values),
                                            (smiles_dataset[dict_smi[DRUG]].cpu()).tile(XTrainGDSC_N.shape[0], 1))
            ...
            PDataset = torch.utils.data.TensorDataset(torch.FloatTensor(XTrainPatients_N),torch.FloatTensor(YTrainPatients.values.astype(int)),
                                            (smiles_dataset[dict_smi[DRUG]].cpu()).tile(XTrainPatients_N.shape[0], 1))
            # sampling source training set from SMOTED data##
            ...
            if extractor == 'FX':
                ...
                Gen = VAE(dropout_gen, IE_dim, h_dim, z_dim)
            elif extractor == 'FX_SFMX':
                Gen = FX_SFMX(dropout_gen, IE_dim, h_dim, z_dim)
            elif extractor == 'FX_MLP':
                Gen = FX_MLP(dropout_gen, IE_dim, h_dim, z_dim)
            elif extractor == 'FX_MCA':
                Gen = MCA(params, dropout_gen, IE_dim, h_dim, z_dim)
            elif extractor == 'FX_MLP_MCA':
                Gen = MCA_MLP(params, dropout_gen, IE_dim, h_dim, z_dim)
            else:
                raise NotImplementedError('{} is not a valid extractor'.format(extractor))
            ...
            scheduler = ReduceLROnPlateau(optimizer_2,patience=5)
            ...
            for it in range(epoch):
                ...
                for i, data in enumerate(zip(cycle(CLoader), PLoader)):
                    ...
                    # SMILES
                    sms = DataS[2].to(device)
                    smt = DataT[2].to(device)
                    ...
                    if extractor == 'FX' or extractor == 'FX_SFMX' or extractor == 'FX_MLP':
                        ...
                        F_xs, recon_xs, mu_s, logvar_s = Gen(xs)
                        F_xt, recon_xt, mu_t, logvar_t = Gen(xt)
                    ...
                    loss3 = 0
                    if extractor == 'FX_VAE':
                        loss_vae_s = loss_vae(xs, recon_xs, mu_s, logvar_s)
                        loss_vae_t = loss_vae(xt, recon_xt, mu_t, logvar_t)
                        loss3 = loss_vae_s + loss_vae_t

                    ### Loss on source training data
                    Loss = loss1 + loss2 + lambdvae * loss3
                ...
                with torch.no_grad():
                    ...
                    smGDSCval = (smiles_dataset[dict_smi[DRUG]].cpu()).tile(TXValGDSC_N.shape[0], 1)
                    ...
                    if extractor == 'FX' or extractor == 'FX_SFMX' or extractor == 'FX_MLP':
                        ...
                        F_xs_val, recon_xs_val, mu_s_val, logvar_s_val = Gen(TXValGDSC_N)

                    elif extractor == 'FX_MCA' or extractor == 'FX_MLP_MCA':
                        F_xs_val = Gen(smGDSCval.to(device), TXValGDSC_N)
                    else:
                        raise NotImplementedError('{} is not a valid extractor'.format(extractor))
                ...
                #scheduler.step(AUCval_source_mean)
                ######
        ...
        ##
        with open(tr_eva_ts_summary_path, 'a') as f:
            f.write("Average Test AUC = {};  path = {}\n".format(avgAUC, test_results_file))
            f.close()
            ##
